...
public interface FestivalsRepository extends JpaRepository<Festivals, Long> {
    ...
    // ✅ 특정 축제 이름으로 월별 개최 횟수 통계
    @Query("""
           select MONTH(f.fstvlBeginDe) as month,
                  count(f) as cnt
           from Festivals f
           where f.fcltyNm = :name
           group by MONTH(f.fstvlBeginDe)
           order by cnt desc
           """)
    List<Object[]> countByMonthForFestivalName(@Param("name") String name);
}
